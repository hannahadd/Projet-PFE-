...
def article_fingerprint(title: str, text: str, n_chars: int = 400) -> str:
    t = norm_text(title)
    lead = norm_text((text or "")[:n_chars])
    raw = (t + "||" + lead).encode("utf-8", errors="ignore")
...
class Article:
    ...
    def from_dict(d: Dict[str, Any]) -> "Article":
        ...
        fp = d.get("fingerprint") or article_fingerprint(title, text)
...
def compute_final_scores(
    ...
    for aid, a in candidates.items():
        ...
        bm25_norm = (bm25_scores.get(aid, 0.0) / (max_bm25 + 1e-9))
...
def main(
    ...
    if not interests:
        interests = [
            "Je veux surtout du football européen et des compétitions UEFA",
            "Je suis la recherche en IA et les LLMs",
            "Politique indienne et géopolitique en Asie"
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("data_path", help="Path to JSON or JSONL dataset")
