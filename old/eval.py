...
def _run_exists(store: PostgresStore, table: str, run_id: int) -> bool:
    if table == "retrieval_hits":
        sql = "SELECT 1 FROM retrieval_runs WHERE id = %s LIMIT 1"
    else:
        sql = "SELECT 1 FROM rerank_runs WHERE id = %s LIMIT 1"
...
def _has_hits_for_run(store: PostgresStore, table: str, run_id: int) -> bool:
    sql = f"SELECT 1 FROM {table} WHERE "
    if table == "retrieval_hits":
        sql += "retrieval_run_id = %s LIMIT 1"
    else:
        sql += "rerank_run_id = %s LIMIT 1"
    with store.connect() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (int(run_id),))
            return cur.fetchone() is not None
...
def _suggest_counterpart_id(store: PostgresStore, table: str, run_id: int) -> Optional[int]:
    # Helpful hint for common mistake: using retrieval_run_id with --table rerank_hits.
    if table != "rerank_hits":
        return None
    sql = "SELECT id FROM rerank_runs WHERE retrieval_run_id = %s ORDER BY id DESC LIMIT 1"
    with store.connect() as conn:
        with conn.cursor() as cur:
            cur.execute(sql, (int(run_id),))
            row = cur.fetchone()
...
def main() -> int:
    ...
    if not _run_exists(store, args.table, run_id):
        if args.table == "rerank_hits":
            hint = _suggest_counterpart_id(store, args.table, run_id)
            if hint is not None:
                raise RuntimeError(
                    f"No rerank run with id={run_id}. You likely passed a retrieval_run_id. "
                    f"Try --table rerank_hits --run-id {hint} instead."
                )
            if _run_exists(store, "retrieval_hits", run_id):
                raise RuntimeError(
                    f"run-id={run_id} is a retrieval_run_id, not a rerank_run_id. "
                    "Run main/reranker.py with --retrieval-run-id first, then evaluate the returned rerank_run_id."
                )
        raise RuntimeError(f"Run id={run_id} not found for table={args.table}.")

    if not _has_hits_for_run(store, args.table, run_id):
        if args.table == "rerank_hits":
            raise RuntimeError(
                f"Run id={run_id} exists in rerank_runs but has no rows in rerank_hits. "
                "Run main/reranker.py first, then evaluate that rerank_run_id."
            )
        raise RuntimeError(
            f"Run id={run_id} exists in retrieval_runs but has no rows in retrieval_hits."
        )

    hits = _fetch_hits(store, args.table, run_id)
