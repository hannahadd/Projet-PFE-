...
<body>
    ...
    <form action="/add-interest" method="post">
        <input type="text" name="new_interest" placeholder="Ex: SpaceX, Finance..." required>
                ...
                {% if articles %}
                    {% for art in articles %}
                        <div class="article-item">
                            ...
                            <small style="color: #666;">Publié le : {{ art[1][:10] }}</small>
                            <p>{{ art[2] }}</p>
                        </div>
                    {% endfor %}
                {% else %}
...
def add_interest():
    ...
    if exists:
        ...
        flash(f"🚀 Pipeline lancé pour '{topic}'.")
...
def show_stories():
    ...
    for interest in selected:
        # Requête sans LIMIT 3 pour tout afficher
        cur.execute("""SELECT title, published_date, summary_fr FROM article_summaries 
                        ...
                        ORDER BY published_date DESC;""", (interest, interest))
